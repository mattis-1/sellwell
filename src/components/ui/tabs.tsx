...
export const Tabs = ({
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "flex flex-row items-center justify-start relative overflow-auto sm:overflow-visible no-visible-scrollbar max-w-full w-full mb-6 sm:mb-8 md:mb-12 px-1 sm:px-0",
          containerClassName
        )}
      >
        {propTabs.map((tab) => (
          <button
            ...
            className={cn(
              "relative px-3 sm:px-4 md:px-6 py-2 sm:py-3 rounded-full mx-1 sm:mx-2 whitespace-nowrap", 
              tabClassName
            )}
          >
            ...
            <span className="relative block text-black dark:text-white font-medium text-xs sm:text-sm md:text-base">
              {tab.title}
            </span>
          </button>
        ))}
      ...
      {/* Tab content with image + text layout - optimized for mobile */}
      <div className={cn("mt-4 sm:mt-6 md:mt-8 relative", contentClassName)}>
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="w-full"
          >
            {/* Image and content layout - optimized for mobile */}
